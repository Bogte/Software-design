...
public class SEditRoomAction extends AbstractRoomAction {
    public SEditRoomAction(){
        putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_MINUS, ActionEvent.SHIFT_MASK));
        putValue(SMALL_ICON, loadIcon("/images/ruler.png"));//postaviti ikonicu
        putValue(NAME, "");
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            (MainFrame.getInstance().getTabs().getTabs().get(MainFrame.getInstance().getTabs().getSelectedIndex())).getRoomView().getStateManager().setEditRoomState();
